class RacesController < ApplicationController
  def index
    ...
    @races = @q.result(:distinct => true).includes(:event, :results, :users).page(params[:page]).per(10)
    @location_hash = Gmaps4rails.build_markers(@races.where.not(:location_latitude => nil)) do |race, marker|
      marker.lat race.location_latitude
      marker.lng race.location_longitude
      marker.infowindow "<h5><a href='/races/#{race.id}'>#{race.name}</a></h5><small>#{race.location_formatted_address}</small>"
    end
